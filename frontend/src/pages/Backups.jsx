...
import api from '../services/api'; // using generic api instance
...
function Backups() {
  ...
  const saveConfig = async (e) => {
    e.preventDefault();
    try {
      await api.post('/core/backup-config/', { frecuencia, hora_ejecucion: hora, activo });
  ...
  const processRestore = async () => {
    try {
      ...
      setRestoring(true);
      const res = await api.post('/core/backups/restore/', { backup_id: selectedBackupId });
  ...
  return (
    <div>
      {restoring && <LoadingScreen message="⚠️ RESTAURANDO BASE DE DATOS... NO CIERRE ESTA PÁGINA ⚠️" />}
      
      <ConfirmDialog
        visible={confirmVisible}
        title="⚠️ ADVERTENCIA CRÍTICA: restauración de Backup"
