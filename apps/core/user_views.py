...
# ─── TEMPORAL: fix empresa de CompraServicio ────────────────────────────────
from rest_framework.decorators import api_view as _apv3, permission_classes as _pc3
from rest_framework.permissions import AllowAny as _AA3
from rest_framework.response import Response as _Rsp3

@_apv3(['POST', 'GET'])
@_pc3([_AA3])
def fix_compra_servicio_empresa(request):
    """Diagnose and fix empresa mismatch in CompraServicio records."""
    secret = (request.data or {}).get('secret') or request.query_params.get('secret', '')
    if secret != 'negocia-fix-2024':
        return _Rsp3({'error': 'Unauthorized'}, status=403)
    ...
    try:
        ...
        perfiles = list(PerfilUsuario.objects.values('id', 'empresa_id', 'rol')[:5])
        ...
        import traceback
        return _Rsp3({'error': str(e), 'tb': traceback.format_exc()[-500:], 'schema': schema}, status=500)
