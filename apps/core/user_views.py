...
from rest_framework.decorators import api_view as _api_view2
from rest_framework.permissions import AllowAny as _AllowAny2
from rest_framework.response import Response as _Response2

@_api_view2(['POST'])
@__import__('rest_framework.decorators', fromlist=['permission_classes']).permission_classes([_AllowAny2])
def seed_compras_servicios_view(request):
    secret = (request.data or {}).get('secret') or request.query_params.get('secret', '')
    if secret != 'negocia-seed-2024':
        return _Response2({'error': 'Unauthorized'}, status=403)
    ...
    try:
        ...
        empresa = Empresa.objects.first()
        if not empresa:
            return _Response2({'error': 'No empresa', 'schema': current_schema}, status=400)
        ...
        if not servicios:
            return _Response2({'error': 'No hay servicios activos', 'schema': current_schema}, status=400)
        ...
        for estado, precio, dias, notas in ejemplos:
            ...
            CompraServicio.objects.create(
                ...
                precio=Decimal(str(precio)), estado=estado, fecha_programada=fecha, notas=notas,
            )
            created += 1
            results.append(f"{serv.nombre} - {estado} - S/.{precio}")
        return _Response2({'message': f'Creados {created}', 'created': created, 'total_now': existing+created, 'items': results, 'schema': current_schema, 'tenant': str(tenant)})
    except Exception as e:
        import traceback
        return _Response2({'error': str(e), 'traceback': traceback.format_exc()[-500:], 'schema': current_schema}, status=500)
